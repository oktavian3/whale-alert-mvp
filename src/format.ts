...
export function formatSignal(signal: Signal): string {
  const c = signal.coin;
  const lines = [
    `🐋 ${arrow[signal.bias]} ${signal.bias} — ${c.name} (${c.symbol.toUpperCase()})`,
...
export function formatTop(signals: Signal[], title = 'Top Signals'): string {
  return [`${title}`, ...signals.slice(0, 10).map((s, i) => `${i + 1}. ${s.coin.symbol.toUpperCase()} ${s.bias} ${s.score}/100 - ${s.signal}`)].join('\n');
}
